...
# 1) faça uma função recursiva que receba um número, uma lista, seu tamanho/comprimento 
# e retorne a quantidade de vezes que o número aparece na lista. DICA: recursao com retorno 
# de valor (return)

def vezes_numero(numero, lista, n):
    if n > 0:
        if lista[n - 1] == numero:
            return 1 + vezes_numero(numero, lista, n - 1)
        else:
            return 0 + vezes_numero(numero, lista, n - 1)
    else:
        return 0
 
# 2) faça uma função recursiva que receba um número, uma lista, seu tamanho/comprimento 
# e substitua o número pelo valor -1. DICA: recursao sem retorno de valor (sem return)

def substituir_numero(numero, lista, n):
    if n > 0:
        if lista[n - 1] == numero:
            lista[n - 1] = -1
        substituir_numero(numero, lista, n - 1)

 
# 3) faça uma função recursiva que receba um número, uma lista, seu tamanho/comprimento 
# e retorne a posição do número na lista (ou -1 se o número não estiver presente). 
# DICA: recursao com retorno de valor (return)
 
def posicao_numero(numero, lista, n):
    if n > 0:
        if lista[n - 1] == numero:
            return n - 1
        else:
            return posicao_numero(numero, lista, n - 1)
    else:
        return -1